...
#各个组织靶基因表达量，注意，只有9个组织的有表达量,没有Placenta的表达量是-1
...
#合并10个组织的eRNA表达量和靶基因表达量到一个文件,添加type和tissue两列：
#创建空文件：
...
###
#补充每个增强子的特异性信息到最后一列并将文件重命名： enh_exp_targetgene_exp_spe
#注意：其中有random的行，
cd /home/ding/all_cmd/script/enh_statistics/
awk 'BEGIN{OFS="\t"}{ if(NR==FNR){a[$4]=$6} if(NR>FNR&&$4~"^enh"){print $0,a[$4]} if(NR>FNR&&!($4~"^enh")){print $0,"random"}}'   ./enh_all_count_spe.bed ./enh_exp_targetgene_exp >./enh_exp_targetgene_exp_spe

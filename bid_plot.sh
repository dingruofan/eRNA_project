...
#enh的中心2kb内有me1/27ac:  ../../DNase/$1/Dnase_ENCODE_cluster.bed
#awk 'BEGIN{OFS="\t"}{ printf "%s\t%d\t%d\n",$1,($2+$3)/2-100,($2+$3)/2+100 }' ../../DNase/$1/Dnase_ENCODE_cluster.bed |
...
#H3K4me3的处理：然而并没有处理：($NF>0||$NF<-0)($NF>=200||$NF<=-200)
...
bwtool agg 2000:2000 ./enh_find/result.bed ./bg/H3K4me1.bw,./bg/H3K4me3.bw,./bg/H3K27ac.bw,../../DNase/$1/new_DHS_TPM.bw  /dev/stdout > ./enh_find/result.mean_signal
now_path=`pwd`
figure_path=/media/ding/000B49000006264C/eRNA_project/figure/histone_DHS/$1
 
Rscript $2/bid_plot.R $1 $now_path $figure_path 
echo $1
echo $figure_path
...
awk '{print $1"\t"$2"\t"$3}' ./enh_find/Dnase_histone_bidir_enh.bed >./enh_find/a1 

#bwtool agg 2000:2000 ./enh_find/a1  ./bg/H3K4me1.bw,./bg/H3K4me3.bw,./bg/H3K27ac.bw  /dev/stdout > ./enh_find/result.mean_signal
now_path=`pwd`

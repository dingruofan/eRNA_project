...
#由于有些TF的名称和基因的名称相同，第三列和第五列。在相同名称的基因后加"-",实际上不在这里加，后面在all_relation里加。
...
#只取出/home/ding/all_cmd/script/enh_statistics/TS_32TF_each_9tissue里的转录因子所对应的组织的关系，其他的不要。
cd /home/ding/all_cmd/script
awk 'BEGIN{OFS="\t"}{ if(NR==FNR&&NR>1){a[$1"\t"$2]=$3} if(NR>FNR&&($3"\t"$4 in a)){ print $0 } }'  ./enh_statistics/TS_32TF_each_9tissue ./enh_statistics/TS_enh_TF_targetgene_3.2  |sort -k 4 -> ./enh_statistics/TS_enh_TF_targetgene_3.3 

awk '{print $4}' ./enh_statistics/TS_enh_TF_targetgene_3.3 |sort |uniq -c
...
############
cd ~/all_cmd/script
tissue=(Breast Liver Lung Placenta SkeletalMuscle Kidney)
#tissue=(Adrenal)
for i in ${tissue[@]}
do
  #先创建空文件：
  ...
  awk -v tissue=$i 'BEGIN{OFS="\t"}{ if($4==tissue){print $3,$5,"TF","gene","TF-gene"} }' ./enh_statistics/TS_enh_TF_targetgene_3.3 |sort -u - >./enh_statistics/cytoscape/a
  ...
  awk -v tissue=$i 'BEGIN{OFS="\t";}{ if($4==tissue){print $1,$3,"enhancer","TF","enhancer-TF"} }' ./enh_statistics/TS_enh_TF_targetgene_3.3 |sort -u - >>/media/ding/000B49000006264C/eRNA_project/cytoscape/$i"_enh_TF_target_relation"
  
  #添加 enh	target	"enhancer"	"gene" 
  #print "enhancer","gene","enh_label","gene_label","relation"
  awk -v tissue=$i 'BEGIN{OFS="\t";}{ if($4==tissue){print $1,$5,"enhancer","gene","enhancer-gene"} }' ./enh_statistics/TS_enh_TF_targetgene_3.3 |sort -u - >>/media/ding/000B49000006264C/eRNA_project/cytoscape/$i"_enh_TF_target_relation"
...
cd ~/all_cmd/script
#先创建总的空文件：
awk 'BEGIN{OFS="\t";print "start","end","start_label","end_label","relation","tissue"}' >/media/ding/000B49000006264C/eRNA_project/cytoscape/all_enh_TF_target_relation

tissue=(Breast Liver Lung Placenta SkeletalMuscle Kidney)
for i in ${tissue[@]}
do
  awk -v tissue=$i 'BEGIN{OFS="\t"} { if(NR>1){print $0,tissue} }' /media/ding/000B49000006264C/eRNA_project/cytoscape/$i"_enh_TF_target_relation" >>/media/ding/000B49000006264C/eRNA_project/cytoscape/all_enh_TF_target_relation
...
do
  ...
  #添加TF	TSVP
  awk -v tissue=$i 'BEGIN{OFS="\t";}{ if($4==tissue){print $3,$8} }' ./enh_statistics/TS_enh_TF_targetgene_3.3 |sort -u - >>/media/ding/000B49000006264C/eRNA_project/cytoscape/$i"_enh_TF_target_value"
  #添加gene	100
  awk -v tissue=$i 'BEGIN{OFS="\t"}{ if($4==tissue){print $5,100} }' ./enh_statistics/TS_enh_TF_targetgene_3.3 |sort -u - >>/media/ding/000B49000006264C/eRNA_project/cytoscape/$i"_enh_TF_target_value"

  #enhancer	信号平均值：来源于上面生成的./enh_statistics/TS_enh_TF_targetgene_3_DHS，表示增强子活性：
  awk -v tissue=$i 'BEGIN{OFS="\t"}{ if($4==tissue){print $1,$9} }' ./enh_statistics/TS_enh_TF_targetgene_3.3 |sort -u - >>/media/ding/000B49000006264C/eRNA_project/cytoscape/$i"_enh_TF_target_value"
...
#合并6个组织的value:
#先创建总的空文件：
awk 'BEGIN{OFS="\t";print "new_start","value","tissue"}' >/media/ding/000B49000006264C/eRNA_project/cytoscape/all_enh_TF_target_value
cd /home/ding/all_cmd/script
tissue=(Breast Liver Lung Ovary Placenta SkeletalMuscle Kidney)
for i in ${tissue[@]}
do
  awk -v tissue="$i" 'BEGIN{OFS="\t"}{ if(NR>1){print $0,tissue} }' /media/ding/000B49000006264C/eRNA_project/cytoscape/$i"_enh_TF_target_value" >>/media/ding/000B49000006264C/eRNA_project/cytoscape/all_enh_TF_target_value
done
###############################################
...
do
  ...
  #添加TF	NR
  awk -v tissue=$i 'BEGIN{OFS="\t";}{ if($4==tissue){print $3,NR} }' ./enh_statistics/TS_enh_TF_targetgene_3.3 |sort -u - >>/media/ding/000B49000006264C/eRNA_project/cytoscape/$i"_enh_TF_target_location"
  #添加gene	NR
  awk -v tissue=$i 'BEGIN{OFS="\t"}{ if($4==tissue){print $5,NR} }' ./enh_statistics/TS_enh_TF_targetgene_3.3 |sort -u - >>/media/ding/000B49000006264C/eRNA_project/cytoscape/$i"_enh_TF_target_location"
  #enhancer	NR
  awk -v tissue=$i 'BEGIN{OFS="\t"}{ if($4==tissue){print $1,NR} }' ./enh_statistics/TS_enh_TF_targetgene_3.3 |sort -u - >>/media/ding/000B49000006264C/eRNA_project/cytoscape/$i"_enh_TF_target_location"
  echo $i
done
...
do
  ...
  #TF edge_num
  awk -v tissue=$i 'BEGIN{OFS="\t"}{ if($4==tissue){ a[$3]++; } }END{ for(i in a){print i,a[i]} }' ./enh_statistics/TS_enh_TF_targetgene_3.3  >>/media/ding/000B49000006264C/eRNA_project/cytoscape/$i"_enh_TF_target_edge_num"
  #gene edge_num
  awk -v tissue=$i 'BEGIN{OFS="\t"}{ if($4==tissue){ a[$5]++; } }END{ for(i in a){print i,a[i]} }' ./enh_statistics/TS_enh_TF_targetgene_3.3 >>/media/ding/000B49000006264C/eRNA_project/cytoscape/$i"_enh_TF_target_edge_num"
  #enh edge_num
  awk -v tissue=$i 'BEGIN{OFS="\t"}{ if($4==tissue){ a[$1]++; } }END{ for(i in a){print i,a[i]} }' ./enh_statistics/TS_enh_TF_targetgene_3.3 >>/media/ding/000B49000006264C/eRNA_project/cytoscape/$i"_enh_TF_target_edge_num"
  echo $i
done

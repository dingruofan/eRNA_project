...
#2kb内有me1/27ac:
closest-features --closest  --delim '\t' --dist ../../DNase/$1/Dnase_ENCODE_cluster.bed ./bed/H3K27ac/H3K27ac_peaks.bed|awk '($NF<=2000&&$NF>=-2000){print $1,$2,$3}' - |sed  's/ /\t/g' - >./enh_find_1/a

closest-features --closest  --delim '\t' --dist ./enh_find_1/a ./bed/H3K4me1/H3K4me1_peaks.bed |awk '($NF<=2000&&$NF>=-2000){print $1,$2,$3}' - |sed  's/ /\t/g' - >./enh_find_1/b
...
closest-features --closest  --delim '\t' --dist ./enh_find_1/c ./bed/H3K4me3/H3K4me3_peaks.bed |awk '{print $1,$2,$3}' - |sed  's/ /\t/g' - >./enh_find_1/result.bed


#处理信号值：要求enh中心附近2kb内H3K4me1的信号的均值大于H3K4me3的信号的均值，没有信号的位置不做统计。(第八列是均值) -header参数查看列名。
bwtool summary ./enh_find_1/result.bed ./bg/H3K4me1.bw -with-sum  /dev/stdout |awk 'BEGIN{OFS="\t"}{print $1,$2,$3,$8}' ->./enh_find_1/H3K4me1_sig_2kb_mean
bwtool summary ./enh_find_1/result.bed ./bg/H3K4me3.bw -with-sum  /dev/stdout |awk 'BEGIN{OFS="\t"}{print $1,$2,$3,$8}' ->./enh_find_1/H3K4me3_sig_2kb_mean
#进行比较：
awk 'BEGIN{OFS="\t"}{ if(NR==FNR){a[$1"\t"$2"\t"$3]=$4}  if(NR>FNR&&a[$1"\t"$2"\t"$3]-$4>0){print $1,$2,$3} }' ./enh_find_1/H3K4me1_sig_2kb_mean ./enh_find_1/H3K4me3_sig_2kb_mean |sortBed -i - >./enh_find_1/result.bed

rm ./enh_find_1/H3K4me1_sig_2kb_mean ./enh_find_1/H3K4me3_sig_2kb_mean

...
for(i in tissues){
  assign(paste(i,"_enh",sep=""),enh_all[enh_all$tissue==i&enh_all$is_spe=="spe",])
  print(i)
}
# 从rGREAT官网建立任务:
for(j in tissues){
...
#选择其中Freq>=8的：
all_tissue_GO_common<-all_tissue_tb_GO[all_tissue_tb_GO$Freq>=8,"name",drop=FALSE]
...
#做图：
png_path="/media/ding/000B49000006264C/eRNA_project/figure/GO_spe_enh_common.png"
...
ggplot(all_tissue_tb_common,aes(x=reorder(tissue,enrich,decreasing=TRUE),y=reorder(name,Binom_Genome_Fraction)))+
  ...
  scale_size("Genome Fraction",breaks=seq(0.1,0.8,0.2))+
  xlab("tissue")+
  ylab("GO name")+
...
#####################################
#以表的形式列出各个spe增强子的GO term:
#每个组织的GO item:
for(j in tissues){
  # 去掉最后一列组织，因为写入文件不需要:
  assign(paste(j,"_tissue_tb",sep=""),get(paste(j,"_tissue_tb",sep=""))[-7])
}
#准备写入xlsx表，创建路径:
xls_path<-"/media/ding/000B49000006264C/eRNA_project/figure/table/GO_spe_tissues.xlsx"
#如果存在，先删掉：
if(file.exists(xls_path)){
  print("文件存在，已经删除")
  file.remove(xls_path)
}
#开始写入：
for(j in tissues){
  write.xlsx2(get(paste(j,"_tissue_tb",sep="")),xls_path,sheetName = j,col.names = TRUE,row.names = FALSE,append=TRUE)
  print(j)
}
